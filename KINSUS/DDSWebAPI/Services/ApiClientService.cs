...
namespace DDSWebAPI.Services
{
    ...
    public class ApiClientService : IDisposable
    {
        ...
        /// <returns>回應結果</returns>
        public async Task<ApiCallResult> SendApiRequestAsync(string endpoint, string requestBody)
        {
            ...
            try
            {
                ...
                {
                    throw new ArgumentException("API 端點不能為空", nameof(endpoint));
                }

                // 如果 endpoint 是完整 URL，直接使用；否則與 _baseUrl 結合
                string url = endpoint.StartsWith("http") ? endpoint : $"{_baseUrl?.TrimEnd('/')}{endpoint}";
                ...
                {
                    ApiCallSuccess?.Invoke(this, new ApiCallSuccessEventArgs(url, requestBody, result.ResponseData));
                }
                else
                {
                    result.ErrorMessage = $"HTTP {result.StatusCode}: {result.ResponseBody}";
                    ApiCallFailure?.Invoke(this, new ApiCallFailureEventArgs(url, requestBody, result.ErrorMessage));
                }
            }
            catch (Exception ex)
            {
                ...
                result.Exception = ex;
                result.ResponseTime = DateTime.Now;                
                ApiCallFailure?.Invoke(this, new ApiCallFailureEventArgs(result.RequestUrl, requestBody, ex.Message, ex));
            }
        ...
        /// <summary>
        /// 發送 POST 請求
        /// </summary>        /// <param name="endpoint">API 端點</param>
        /// <param name="request">請求物件</param>
        /// <returns>回應結果</returns>
        private async Task<ApiCallResult> SendPostRequestAsync(string endpoint, object request)
        {
            ...
            try
            {
                ...
                jsonContent = JsonConvert.SerializeObject(request, Formatting.Indented);
                ...
                {
                    OnApiCallSuccess(endpoint, jsonContent, result.ResponseBody);
                }
                else
                {
                    result.ErrorMessage = $"HTTP 錯誤: {response.StatusCode} - {response.ReasonPhrase}";
                    OnApiCallFailure(endpoint, jsonContent, result.ErrorMessage);
                }
            }            catch (TaskCanceledException ex) when (ex.InnerException is TimeoutException)
            {
                ...
                result.Exception = ex;
                OnApiCallFailure(endpoint, jsonContent, result.ErrorMessage, ex);
            }
        ...
        /// 觸發 API 呼叫成功事件
        /// </summary>
        private void OnApiCallSuccess(string endpoint, string request, string response)
        {
            ApiCallSuccess?.Invoke(this, new ApiCallSuccessEventArgs(endpoint, request, response));
        }

        /// <summary>
        /// 觸發 API 呼叫失敗事件
        /// </summary>
        private void OnApiCallFailure(string endpoint, string request, string error, Exception exception = null)
        {
            ApiCallFailure?.Invoke(this, new ApiCallFailureEventArgs(endpoint, request, error, exception));
        }
    ...
    public class ApiCallResult
    {
        ...
        /// 回應資料（用於相容性）
        /// </summary>
        public string ResponseData => ResponseBody;    }
