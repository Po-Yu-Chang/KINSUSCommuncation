...
namespace DDSWebAPI.Tests.Services
{
    ...
    public class PersistentConnectionTests
    {
        ...
        {
            // Arrange & Act
            bool started = await _apiService.StartServerAsync(TEST_PORT);
        ...
        {
            // Arrange
            await _apiService.StartServerAsync(TEST_PORT);
        ...
        /// 輔助方法：測試 API 端點
        /// </summary>
        private async Task<bool> TestApiEndpoint(string endpoint)
        {
            try
            {
                ...
                {
                    // 測試基本連線
                    return _apiService.IsServerRunning;
                }
                else
                {
